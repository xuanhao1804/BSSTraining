...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  if (isEdit) {
    try {
      ...
      if (
        rule.applyTo === "specific-products" &&
        (rule.productIds || (rule as any).variantIds)
      ) {
        try {
          ...
          const productIds = Array.isArray(rule.productIds)
            ? rule.productIds
            : rule.productIds
              ? JSON.parse(rule.productIds as string)
              : [];

          if (variantIds && variantIds.length > 0) {
            // Skip variant handling for now
          } else if (productIds && productIds.length > 0) {
            const { getProductsByIds } = await import(
      ...
      if (rule.applyTo === "product-collections" && rule.collectionIds) {
        try {
          const collectionIds = Array.isArray(rule.collectionIds)
            ? rule.collectionIds
            : JSON.parse(rule.collectionIds as string);

          if (collectionIds.length > 0) {
            const { getCollectionsByIds } = await import(
              "../services/api.graphql"
            );
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  try {
    ...
    return json(
      {
        errors: {
          general: "Authentication failed. Please refresh and try again.",
        },
      },
      { status: 401 },
    );
  ...
  try {
    ...
    if (isEdit) {
      ...
      if (applyTo === "specific-products") {
        ...
        (updateData as any).variantIds =
          parsedVariantIds || (currentRule as any).variantIds;
        updateData.collectionIds = null;
        updateData.tagIds = null;
      } else if (applyTo === "product-collections") {
        updateData.collectionIds =
          parsedCollectionIds || currentRule.collectionIds;
    ...
    return json(
      {
        errors: {
          general: `Failed to ${isEdit ? "update" : "create"} pricing rule. Please try again.`,
        },
      },
      { status: 400 },
    );
...
export default function PricingRuleForm() {
  ...
  const {
    fields: { name, priority, status, applyTo, priceType, amount },
    submit,
    submitting,
    dirty,
  } = useForm({
    ...
    onSubmit: async (fieldValues) => {
      if (
        fieldValues.applyTo === "specific-products" &&
      ...
      if (
        fieldValues.applyTo === "product-collections" &&
  ...
  useEffect(() => {
    if (rule && isEdit) {
      ...
      const initialTags = Array.isArray(rule.tagIds)
        ? rule.tagIds
        : rule.tagIds
          ? JSON.parse(rule.tagIds as string)
          : [];
  ...
  const hasUnsavedChanges = useCallback(() => {
    ...
    const collectionIdsChanged =
      JSON.stringify(selectedCollections.map((c) => c.id).sort()) !==
  ...
  const handleSpecificProductsChange = () => {
    ...
    if (
      ...
      rule &&
      rule.applyTo === "specific-products" &&
      originalState.selectedProducts.length > 0
    ) {
  ...
  const handleProductCollectionsChange = () => {
    ...
    if (
      isEdit &&
      rule &&
      rule.applyTo === "product-collections" &&
      originalState.selectedCollections.length > 0
    ) {
  ...
  const handleProductTagsChange = () => {
    ...
    if (
      isEdit &&
      rule &&
      rule.applyTo === "product-tags" &&
      originalState.selectedTags.length > 0
    ) {
                  ...
                  <BlockStack gap="400">
                    ...
                    <BlockStack gap="200">
                      ...
                      <RadioButton
                        ...
                        onChange={() =>
                          priceType.onChange("decrease-percentage")
                        }
                      />
                    </BlockStack>
